...
@logger.catch()
def convert_pillow_to_numpy(image_pil: Image, astype: Optional[type] = None) -> np.ndarray:
    """Converts a Pillow image object to a NumPy array.

    Args:
        `image_pil` (Image): The Pillow image object.
        `astype` (type, optional): The desired data type for the NumPy array. Defaults to None.

    Returns:
        np.ndarray: The NumPy array representation of the image.

    Raises:
        Exception: If there is an error converting the Pillow Image object to a NumPy array.
    ...
    """
    try:
        img_array = np.array(image_pil) # <= NOTE convert PIL images into NumPy arrays
        checker_data = '1-D array' if img_array.ndim == 1 else '2-D array' if img_array.ndim == 2 else '3-D array' if img_array.ndim == 3 else None # <= NOTE check array is 3D & 2D & 1D
...
@logger.catch()
def convert_pillow_to_cv2(image_array: np.ndarray) -> np.ndarray:
...
@logger.catch()
def convert_cv2_to_pillow(image_array: np.ndarray) -> np.ndarray:
